...
public class JavaCodeInterfaceConfigCreate implements GenerateCodeInf {
     ...
     * 方法加载
     */
    private static final String LOADER_METHOD_CODE = "        for (Locale locale : CommonSysConfig.LOCALE) {\n" +
    ...
    public void generateCode(GenerateCodeContext param) {
        ...
        // 进行存储层的接口输出
        GenerateOutFileUtils.outJavaFile(outData, GeneratePathUtils.outServicePath(param),
                javaPackageStr,
    ...
    private String methodDefineStart() {
        ...
        JavaMethodEntity methodEntity =
                JavaMethodEntity.builder()
                        // 方法注解
                        .annotation(ImportCodePackageKey.API_CONFIG_LOAD_ANNOTATION.getPackageInfo().getAnnotation())
                        // 访问修饰符
    ...
    private StringBuilder classDefine(GenerateCodeContext param) {
        ImportPackageInfo errorCodeConfigPkg =
                new ImportPackageInfo(param.getJavaCodePackage().getInterfaceConfigNode().outJavaPackage(),
                        getName(param.getModuleName()),
        ...
        JavaClassEntity classEntityInfo =
                JavaClassEntity.builder()
                        // 注解
                        .annotationList(Arrays.asList(ImportJunitPkgKey.SPRING_CONFIG.getPackageInfo().getAnnotation()))
                        // 类的关键字
                        ...
                        //继承类信息
                        .interfaceClass(ImportCodePackageKey.API_CONFIG_INTERFACE.getPackageInfo().getClassName())
                        .build();
    ...
    private List<String> importList(GenerateCodeContext param) {
        ...
        dataImportList.add(ImportCodePackageKey.API_CONFIG_LOAD_ANNOTATION.getPackageInfo().packageOut());
    ...
    private List<ImportPackageInfo> getErrorCodeEnum(Map<String, Map<String, ImportPackageInfo>> packageMap) {
    ...
    private String enumModuleName(GenerateCodeContext param) {
        ...
        for (Map.Entry<String, Map<String, ImportPackageInfo>> itemPkg : param.getPackageMap().entrySet()) {
    ...
    private String getName(String name) {
        return ERROR_CODE_LOAD_PREFIX + NameProcess.INSTANCE.toJavaNameFirstUpper(name) + ERROR_CODE_SUFFIX;
    }
