...
public class GenerateJavaAction {
  ...
  /** 数据查询,无参查询 */
  private static final String HTTP_ONE = "RequestMethod.GET";
  ...
  public StringBuilder generateAction(
    ...
    for (MethodInfo methodItem : methodList) {
      ...
      if (MethodTypeEnum.INSERT.getType().equals(methodItem.getOperator())) {
        GenerateJavaAction.INSTANCE.updateMethod(
            sb,
            methodItem,
            assembler,
            JavaVarName.APPLICATION_INSTANCE_NAME,
      ...
      // 如果当前为查询分页操作
        ...
        GenerateJavaAction.INSTANCE.pageQueryMethod(
            sb,
            methodItem,
            apiObject,
            JavaVarName.APPLICATION_INSTANCE_NAME,
      ...
      // 如果当前为查询则进行查询调用操作
        ...
        GenerateJavaAction.INSTANCE.queryMethod(
            sb,
            methodItem,
            domainPackageInfo,
            JavaVarName.APPLICATION_INSTANCE_NAME,
  ...
  private List<JavaMethodArguments> batchArgumentList(
    ...
    if (checkBatchFlag) {
      ...
      // httpRequest对象
      JavaMethodArguments httpRequest =
          JavaMethodArguments.builder()
              .type(ImportCodePackageKey.HTTP_SERVLET_REQUEST.getPackageInfo().getClassName())
              .name(ImportCodePackageKey.HTTP_SERVLET_REQUEST.getPackageInfo().getVarName())
              .comment(ImportCodePackageKey.HTTP_SERVLET_REQUEST.getPackageInfo().getClassComment())
              .build();

      argumentsList = Arrays.asList(dataTransferArgs, httpRequest);
    }
    // 非批量添加操作
    else {
      ...
      argumentsList = Arrays.asList(dataTransferItem, httpRequest);
    }
  ...
  private List<String> annotation(ImportPackageInfo interfaceFacade) {
    ...
    // slf4j的操作
    JavaAnnotation slf4j = JavaAnnotation.builder().annotation(CodeAnnotation.SL4J).build();
    ...
    outAnnotationList.add(slf4j.outAnnotation());
