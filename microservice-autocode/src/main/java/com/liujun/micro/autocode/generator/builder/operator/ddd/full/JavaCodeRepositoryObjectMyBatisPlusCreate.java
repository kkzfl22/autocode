...
import com.liujun.micro.autocode.generator.builder.operator.code.GenerateMyBatisPlusJavaBean;
 ...
 * @since 2020/04/08
 */
public class JavaCodeRepositoryObjectMyBatisPlusCreate implements GenerateCodeInf {

  /** 注释中的描述 */
  private static final String DOC_ANNOTATION = "mybatis-plus的数据库存储实体信息";

  /** 用来生成存储层的实体名称 */
  public static final String PERSIST_MYBATIS_PLUS_PO = "PO";

  /** 实例对象 */
  public static final JavaCodeRepositoryObjectMyBatisPlusCreate INSTANCE =
      new JavaCodeRepositoryObjectMyBatisPlusCreate();
    ...
    while (tableNameEntry.hasNext()) {
      ...
      ImportPackageInfo packageInfo =
          ImportPackageUtils.getDefineClass(
              param.getPackageMap(),
              GenerateCodePackageKey.PERSIST_MYBATIS_PLUS_PO.getKey(),
              tableName);
      ...
      ImportPackageInfo repositoryObject =
          param
              .getPackageMap()
              .get(tableName)
              .get(GenerateCodePackageKey.PERSIST_MYBATIS_PLUS_PO.getKey());

      // 进行存储层的bean代码生成
      StringBuilder persistBean =
          GenerateMyBatisPlusJavaBean.INSTANCE.generateJavaBean(
  ...
  public void repositoryObjectDependency(
    ...
    String javaPackageStr =
        param.getJavaCodePackage().getMybatisPlusRepositoryObjectNode().outJavaPackage();
  ...
  public String getClassName(String tableName) {
    ...
    return tableClassName + PERSIST_MYBATIS_PLUS_PO;
  }
}
