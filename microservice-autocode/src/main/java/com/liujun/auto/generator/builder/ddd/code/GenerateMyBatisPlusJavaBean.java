...
public class GenerateMyBatisPlusJavaBean {
  ...
  public StringBuilder generateJavaBean(DataParam generateParam) {
    ...
    StringBuilder sb =
        JavaClassCodeUtils.classDefine(
            entityInfo, getImportList(codeMethod, columnList, typeEnum), annotationList, author);
   ...
   * @return 表名输出
   */
  private String tableName(TableInfoDTO tableInfo) {
    StringBuilder outTableName = new StringBuilder();

    outTableName.append(
        ImportCodePackageKey.MYBATIS_PLUS_TABLE_NAME.getPackageInfo().getAnnotation());
    outTableName.append(Symbol.BRACKET_LEFT);
    outTableName.append(Symbol.QUOTE);
    outTableName.append(tableInfo.getTableName());
    outTableName.append(Symbol.QUOTE);
    outTableName.append(Symbol.BRACKET_RIGHT);

    return outTableName.toString();
  }
  ...
  public List<JavaClassFieldEntity> propertiesToJavaEntity(
    ...
    for (int i = 0; i < columnList.size(); i++) {
      ...
      JavaClassFieldEntity field =
          JavaClassFieldEntity.builder()
              // 生成注解
              .annotation(annotation(tableBean))
              // 访问修饰符
              .visit(JavaKeyWord.PRIVATE)
              // 数据类型
              .type(javaDataType)
  ...
  private String annotation(TableColumnDTO tableBean) {
    StringBuilder outData = new StringBuilder();

    outData.append(ImportCodePackageKey.MYBATIS_PLUS_TABLE_FIELD.getPackageInfo().getAnnotation());
    outData.append(Symbol.BRACKET_LEFT);
    outData.append(CodeAnnotation.SWAGGER_API_VALUE);
    outData.append(Symbol.EQUAL);
    outData.append(Symbol.QUOTE);
    outData.append(tableBean.getColumnName());
    outData.append(Symbol.QUOTE);
    outData.append(Symbol.BRACKET_RIGHT);
