package com.liujun.auto.algorithm.ahocorsick;

import com.liujun.auto.algorithm.constant.CharConfig;
 ...
 * @date 2019/03/08
 */
public abstract class BaseAhoCorasick {
  ...
  public Map<String, Integer> matchMult(String src) {
    ...
    Map<String, Integer> matchMap = new HashMap<>(8);
  ...
  private int getIgnoreCaseIndex(char mainChar) {
    ...
    if (index > AC_SIZE) {
      ...
      if (index >= CharConfig.UPPER_CASE_START && index <= CharConfig.UPPER_CASE_END) {
        return index + CharConfig.UPPER_TO_LOWER;
      }
    }
